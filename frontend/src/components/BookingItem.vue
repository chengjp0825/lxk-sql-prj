<template>
  <div class="glass rounded-xl border p-5 hover:border-white/10 transition-all duration-200" style="border-color: var(--border-subtle);">
    ...
    <p v-if="booking.reason" class="text-sm mt-1.5 pl-10" style="color: var(--text-muted);">
      &#x1F4DD; {{ booking.reason }}
    </p>
    <p v-if="booking.admin_reason" class="text-sm mt-1.5 pl-10" style="color: var(--text-muted);">
      &#x1F4AC; Admin: {{ booking.admin_reason }}
    </p>
    <div v-if="canCancel" class="mt-3 pl-10">
      <button @click="handleCancel" class="text-sm text-rose-400 hover:text-rose-300 font-medium transition-colors">Cancel</button>
    </div>
  </div>
</template>
...
const statusClass = computed(() => {
  switch (props.booking.status) {
    ...
    case 'expired': return 'text-xs font-bold bg-slate-500/10 text-slate-500 px-2.5 py-1 rounded-full border border-white/5'
